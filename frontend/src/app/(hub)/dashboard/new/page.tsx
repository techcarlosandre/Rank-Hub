...
export default function NewRankingPage() {
  ...
  const handleAIProcess = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Chamando API: http://127.0.0.1:5000/api/generate-rules");
      const response = await fetch('http://127.0.0.1:5000/api/generate-rules', {
        method: 'POST',
      ...
      const saveRes = await fetch('http://127.0.0.1:5000/api/rankings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      console.log("Ranking criado! Redirecionando para:", newRanking.id);
      router.push(`/r/${newRanking.id}`);
    } catch (err: any) {
      console.error("FALHA TOTAL NO PROCESSO:", err);
